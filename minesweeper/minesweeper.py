...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        # if the number of cells is equal to the number of known mines, all cells must be mines
        mines = set()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        # create list of adjacent cells to add to sentence
        adj_cells = set()
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if (i, j) == cell:
                    continue
                # Ignore adjacent cells that are outside game board boundaries
                if not(0 <= i < self.height) or not(0 <= j < self.width):
                    continue
                # Ignore cells that are in known_mines or in known_safes
                if (i, j) in self.mines:
                    count -= 1
                    continue
                if (i, j) in self.safes:
                    continue
                # Create sentence for cell and its count
                adj_cells.add((i, j))
        
        unknown_cells = Sentence(adj_cells, count)
        self.knowledge.append(unknown_cells)
        
        knowledge_changed = True

        while knowledge_changed:
            knowledge_changed = False
            # check for certainties regarding mines/safes and add any to knowledge base
            mines = set()
            safes = set()
